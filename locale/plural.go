...
// returned.
func (r PluralRules) Iter(iter func(PluralRule)) {
	rel := r.rel
	if len(rel) == 0 {
		return
	}
	...
	for {
		rule := PluralRule{
			...
			Ranges:     Ranges{r: rel.ranges()},
			Connective: Connective(rel.connective()),
		}
		iter(rule)
		if rule.Connective == None {
			return
		}
		rel = rel.next()
	}
...
func (r Ranges) At(i int) Range {
	return Range{LowerBound: int(r.r[2*i]), UpperBound: int(r.r[2*i+1])}
}
...
const (
	...
	CompactDecimalExp   Operand = 6 // c, e
)
