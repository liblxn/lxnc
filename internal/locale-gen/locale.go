...
func (l *locale) generate(p *printer) {
	...
	p.Println(`		return 0, errorf("unsupported language: %s", p.lang)`)
	p.Println(`	case len(p.script) != 0 && script == 0:`)
	p.Println(`		return 0, errorf("unsupported script: %s", p.script)`)
	p.Println(`	}`)
	p.Println()
	...
	p.Println(`			return 0, errorf("unsupported region: %s", p.region)`)
	p.Println(`		}`)
	p.Println()
	...
	p.Println(`		return 0, errorf("locale not found: %s", tag)`)
	p.Println(`	}`)
	...
	p.Println(`	if tag == "" {`)
	p.Println(`		return errorString("empty locale tag")`)
	p.Println(`	}`)
	p.Println()
	p.Println(`	p.s = tag`)
	p.Println(`	p.idx = 0`)
	...
	p.Println(`			return errorf("malformed locale tag: %s", tag)`)
	p.Println(`		}`)
	p.Println(`		return errorf("invalid language subtag: %s", p.tok)`)
	p.Println(`	}`)
	p.Println(`	p.script = p.parseScript()`)
	p.Println(`	p.region = p.parseRegion()`)
	p.Println()
	p.Println(`	if len(p.tok) != 0 {`)
	p.Println(`		return errorf("unsupported locale suffix: %s", p.s[p.idx:])`)
	p.Println(`	}`)
...
func (l *locale) generateTest(p *printer) {
	...
	for _, tag := range parentTags {
		...
		for _, containment := range containments {
			localeContainments = append(localeContainments, containment)
		}
	}
	...
	l.tags.iterate(func(id cldr.Identity) {
		...
		p.Println(`		`, tagID, `: [4]string{"`, id.Language, `", "`, id.Script, `", "`, id.Territory, `", "`, id.String(), `"},`)
	})
